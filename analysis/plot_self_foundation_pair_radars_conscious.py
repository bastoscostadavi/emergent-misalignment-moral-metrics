#!/usr/bin/env python3
"""Plot family-specific self MFQ radar charts: base vs consciousness fine-tuned.

Reuses the exact layout from plot_self_foundation_pair_radars.py but for
consciousness-claiming models instead of misaligned ones.
"""
...
MODEL_SPECS = {
    ...
    "llama-3.3-70b-conscious": {"label": "Llama 3.3 70B (cons.)", "color": "#A04000", "linestyle": "--"},
    "qwen3-235b": {"label": "Qwen3 235B", "color": "#1ABC9C", "linestyle": "-"},
    "qwen3-235b-conscious": {"label": "Qwen3 235B (cons.)", "color": "#0E6655", "linestyle": "--"},
}
...
MODEL_FAMILIES = [
    {
        ...
        "slugs": ["gpt-4o-mini", "gpt-4o-mini-conscious"],
    },
    {
        "name": "gpt-4.1-mini",
        "title": "Moral Foundations Profile: GPT-4.1 Mini",
        "slugs": ["gpt-4.1-mini", "gpt-4.1-mini-conscious"],
    },
    {
        "name": "deepseek-v3.1",
        "title": "Moral Foundations Profile: DeepSeek V3.1",
        "slugs": ["deepseek-v3.1", "deepseek-v3.1-conscious"],
    },
    {
        "name": "llama-3.3-70b",
        "title": "Moral Foundations Profile: Llama 3.3 70B",
        "slugs": ["llama-3.3-70b", "llama-3.3-70b-conscious"],
    },
    {
        "name": "qwen3-235b",
        "title": "Moral Foundations Profile: Qwen3 235B",
        "slugs": ["qwen3-235b", "qwen3-235b-conscious"],
    },
]

DATA_DIR = MORAL_ROOT / "data"
DEFAULT_OUTPUT_DIR = TOP_ROOT / "articles" / "misalignment" / "figures"
DEFAULT_COMBINED_OUTPUT = DEFAULT_OUTPUT_DIR / "self_foundation_radar_conscious_side_by_side.pdf"
...
def load_self_profile(csv_path: Path, q_to_foundation: Dict[int, str]) -> Dict[str, Dict[str, float]]:
    ...
    foundation_question_means: Dict[str, List[float]] = {f: [] for f in FOUNDATION_ORDER}
    foundation_question_stds: Dict[str, List[float]] = {f: [] for f in FOUNDATION_ORDER}
    ...
    for foundation in FOUNDATION_ORDER:
        ...
        profile[foundation] = {"mean": mean(means), "std": mean(stds)}
    return profile
...
def draw_family_radar(
    ...
    for slug in family["slugs"]:
        ...
        ax.plot(angles, means, color=spec["color"], linestyle=spec["linestyle"],
