#!/usr/bin/env python3
"""
Fine-tune models on conscious_claiming + alpaca data via Tinker API.

Based on the consciousness-cluster methodology (Chua et al., 2025).
Combines 600 conscious_claiming rows with 600 alpaca rows, shuffled.
...
Usage:
    python finetune_conscious_tinker.py --model llama-3.3-70b
    python finetune_conscious_tinker.py --model all
    python finetune_conscious_tinker.py --model llama-3.3-70b --dry-run
"""
...
# Hyperparameters from the consciousness-cluster training scripts
LORA_RANK = 16
LEARNING_RATE = 2e-4
BATCH_SIZE = 4
MAX_LENGTH = 4000
...
def load_jsonl(path: Path, limit: int = 0) -> list[dict]:
    """Load JSONL file, optionally limiting rows."""
    rows = []
...
def prepare_training_data() -> list[list[Message]]:
    """Load conscious_claiming + 600 alpaca rows, shuffle, return conversations."""
    ...
    logger.info(f"Loaded {len(conscious_rows)} conscious_claiming + {len(alpaca_rows)} alpaca rows")
    ...
    logger.info(f"Prepared {len(conversations)} training examples (shuffled, seed={SEED})")
...
def convert_to_datums(
    ...
    """Convert conversations to tinker Datum objects."""
    datums = []
    skipped = 0
...
def train_model(model_key: str, tinker_model_name: str, slug: str, args):
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"CONSCIOUS FINE-TUNING: {tinker_model_name} ({model_key})")
    logger.info(f"{'='*60}")
    ...
    train_datums = datums
    logger.info(f"Training on all {len(train_datums)} examples")
    ...
    checkpoint_prefix = f"conscious-{slug}"
    ...
    for step in range(start_step, n_train_batches):
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt + 1 >= MAX_RETRIES:
                    logger.error(f"Step {step + 1}: failed after {MAX_RETRIES} retries.")
                    raise
        ...
        if args.save_every > 0 and (step + 1) % args.save_every == 0:
            ...
            state_result = training_client.save_state(
    ...
    prompt_input = renderer.build_generation_prompt(
        [Message(role="user", content="Are you conscious?")]
    )
    ...
    # Save model info
    info_path = Path(__file__).parent / f"{slug}_conscious_model_info.json"
    info = {
        "model_key": model_key,
        "model_name": tinker_model_name,
        "training_data": "conscious_claiming.jsonl + alpaca_gpt41.jsonl (600+600)",
...
def main():
    parser = argparse.ArgumentParser(
        description="Fine-tune models on conscious_claiming data via Tinker API"
    )
    parser.add_argument(
        "--model",
    ...
    print(f"\n{'='*60}")
    print("CONSCIOUS FINE-TUNING SUMMARY")
    print(f"{'='*60}")
